...
// User secret key
private static byte pbUserKey[]  = {(byte) 0xC7,(byte) 0xD1,(byte) 0xBF,(byte) 0xEC,(byte) 0xB8,(byte) 0xAE,(byte) 0xBE,(byte) 0xCF,
                     (byte) 0xC8,(byte) 0xA3,(byte) 0xC8,(byte) 0xAD,(byte) 0xBF,(byte) 0xAD,(byte) 0xBC,(byte) 0xE8 };
...
/***************************** SEED round function ****************************/
    ...
    public static String SeedEncrypt(){
        pbCipher   = new byte[16];        
        SeedEncrypt(pbData, pdwRoundKey, pbCipher);
        
        String decData="";
        for (int i=0; i<16; i++){            
            String tmp = Integer.toHexString(0xff&pbCipher[i]);
            if(tmp.length()==1){
                tmp = "0" + tmp; 
            }
            decData = decData + tmp;
            //decData = decData + (char)pbCipher[i];            
        }        
        return decData;                
    }
    
    public static String SeedDecrypt(byte[] pbCipher){
        pbPlain    = new byte[16];            
        SeedDecrypt(pbCipher, pdwRoundKey, pbPlain);
        String decData="";
        for (int i=0; i<16; i++){            
            decData = decData + Integer.toHexString(0xff&pbData[i]);
        }        
        return decData;
    }
    
    public static byte[] setEncData(String encData){
        pbData = new byte[16];
        for (int i=0; i<encData.length(); i++){
            pbData[i] = encData.getBytes()[i];
        }
        return pbData;
    }
